...
/**
 * This makes sure that all commands executed (when invocation assistance is
 * enabled) are run through cov-build.
 */
@Extension
public class CoverityLauncherDecorator extends LauncherDecorator {

    private static final Logger logger = Logger
            .getLogger(CoverityLauncherDecorator.class.getName());
    /**
     * A ThreadLocal that is used to disable cov-build when running other
     * Coverity tools during the build.
     */
    ...
    public Launcher decorate(Launcher launcher, Node node) {
        ...
        try {
            if (ii == null) {
                ...
                temp = new FilePath(node.getChannel(), ii.getIntermediateDir() + build.getNumber());

                logger.info("mkdir temp: " + temp.getRemote());

                temp.mkdirs();
            }

            if (ii != null) {
                build.addAction(new CoverityTempDir(temp, ii
                        .getIntermediateDir() == null));
            } else {
                build.addAction(new CoverityTempDir(temp, true));
            }
        } catch (IOException e) {
            throw new RuntimeException(
                    "Error while creating temporary directory for Coverity", e);
        } catch (InterruptedException e) {
            throw new RuntimeException(
                    "Interrupted while creating temporary directory for Coverity");
        }
        ...
        for (CIMStream cs : publisher.getCimStreams()) {
            if (!cs.getDefectFilters().checkConfig()) {
                throw new RuntimeException(
                        "Defect Filters Configured incorrectly. Possibly new configuration have been "
                                + "added and needs configurations. \n Please check your build configuration before running another build.");
            }
            CIMInstance cim = publisher.getDescriptor().getInstance(
                    cs.getInstance());
            ...
            try {
                ...
                throw new RuntimeException(
                        "Error while retrieving stream information for instance/stream: "
                                + id, e);
            } catch (IOException e) {
                throw new RuntimeException(
                        "Error while retrieving stream information for instance/stream: "
        ...
        if (publisher.getCimInstance() != null) {
            CIMInstance cim = publisher.getDescriptor().getInstance(
                    publisher.getCimInstance());
            String id = publisher.getCimInstance() + "/"
                    + publisher.getStream();
            try {
        ...
        CoverityVersion version = CheckConfig.checkNode(publisher, build,
                launcher, launcher.getListener()).getVersion();
        ...
        /**
         * Putting together the arguments for cov-build. The placeholder is
         * there so that we can later determine the exact location of cov-build
         * runable.
         */
        ...
        // Adding in Test Analysis arguments into the code when no other test
        // command is needed to run.
        if (ta != null) {
        ...
        return new DecoratedLauncher(launcher, blacklist, node,
                args.toArray(new String[args.size()]));
    }

    /**
     * A decorated {@link Launcher} that puts the given set of arguments as a
     * prefix to any commands that it invokes.
     */
    public class DecoratedLauncher extends Launcher {
        ...
        public DecoratedLauncher(Launcher decorated, String[] blacklist,
        ...
        public Proc launch(ProcStarter starter) throws IOException {
            if (!SKIP.get()) {
                if (isBlacklisted(starter.cmds().get(0))) {
                    logger.info(starter.cmds().get(0)
                            + " is blacklisted, skipping cov-build");
                ...
                if (lastArg.startsWith(toolsDir) && lastArg.endsWith(".sh")) {
                    logger.info(lastArg
                            + " is a tools script, skipping cov-build");
                    return decorated.launch(starter);
                }
        ...
        public Channel launchChannel(String[] cmd, OutputStream out,
                FilePath workDir, Map<String, String> envVars)
        ...
        /*
         * When running remotely, overriding this method makes sure that the
         ...
         * getCovBuild
         * 
         * Retrieves the location of cov-build executable/sh from the system and
        ...
        private String getCovBuild() {
            ...
            AbstractBuild build = (AbstractBuild) exec;
            AbstractProject project = build.getProject();
            CoverityPublisher publisher = (CoverityPublisher) project
                    .getPublishersList().get(CoverityPublisher.class);
            InvocationAssistance ii = publisher.getInvocationAssistance();
            ...
            String home = null;
            try {
                home = publisher.getDescriptor().getHome(node,
            ...
            if (ii != null) {
                if (ii.getSaOverride() != null) {
                    try {
                        home = new CoverityInstallation(ii.getSaOverride())
            ...
            if (home != null) {
                covBuild = new FilePath(node.getChannel(), home).child("bin")
                        .child(covBuild).getRemote();
            }
